...
public interface CourseDao extends JpaRepository<Course, Long> {
    ...
    @Query(value = "select * from course where course_id in (select e.course_id from enrolled_in as e where e.student_id=:studentId)",
            nativeQuery = true)
    Page<Course> getCoursesByStudentId(@Param("studentId") Long studentId, Pageable pageable);

    @Query(value = "select * from course where course_id not in (select e.course_id from enrolled_in as e where e.student_id=:studentId)",
            nativeQuery = true)
    Page<Course> getNonEnrolledCoursesByStudentId(@Param("studentId") Long studentId, Pageable pageable);
